...
- **Facebook feed.** The Facebook page block reserves the embed’s height and inserts the Page Plugin iframe only when the block comes within a screen of the viewport, so the homepage does not download the embed on load. A `<noscript>` copy keeps it working without JavaScript.
